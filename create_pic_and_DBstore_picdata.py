...
def get_tushare_data(symbol):
    ...
    try:
        ...
        df = df[34:]
        df.to_csv(path_or_buf='E:\pic_data\datacsv\\'+symbol+'.csv',sep=',',index=True)
    except:
        print(symbol)
        print(df)
...
def deal_close_datas(symbol,df,sequence):
    ...
    for j in range(len(comp_days_close)):
        ...
        # print(days_close[j])
        if days_close[j] - close > 0:
            comp_days_close[j] = 1
        elif days_close[j] - close == 0:
            comp_days_close[j] = 0
        else:
            comp_days_close[j] = -1
...
def create_kdj_pic(symbol,df,sequence):
    ...
    try:
        ...
        plt.axis('off')
        # plt.show()
        fig.savefig('E:\pic_data\kdj_pic\\'+symbol+'\\'+str(sequence))
        plt.close()
        return 1
    except:
...
def create_macd_pic(symbol,df,sequence):
    ...
    try:
        ...
        fig.savefig('E:\pic_data\macd_pic\\' + symbol + '\\' + str(sequence))
        plt.close()
        return 1
    except:
...
def create_pics_and_datas(symbol,pic_type=None):
    data_dict_list = []
    df = pd.read_csv('E:\pic_data\datacsv\\' + symbol + '.csv', sep=',')
    ##计算kdj
